...
# Can have multiple implementations (e.g. clients may want to save to disk and dedicated server to cloud object storage).
# Game saves are identified by an ID. Essentially this ID is a game session (a new "run").
# There can only be one save file per session (because roguelike, duh).
extends Node
