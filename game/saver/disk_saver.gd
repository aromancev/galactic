...
# 
# Each game save is represented by one file.
# Files are named using the pattern: [time]_[id]. The time format is lexicographically ordered.
# Since file names are returned in alphabetical order, this means save files are ordered by creation 
# time and we don't need to actually read the files to get the latest, for example.
# 
# The code itself SHOULD NOT know or rely on any specific time format. If you need to display save creation
# time in age, put it in the file itself.
...
func save_game(game: SavedGame) -> UUID:
    ...
    # Remove the old save files if it exist.
    # If the game crushes after renaming the new save but before removing the old one, we'll have an extra 
    # save file.
    # This is extremely unlikely though and is better than potentially losing a save.
    # The extra file will be cleaned up by `check_and_repair` or `save_game`.
...
func load_game(id: UUID) -> SavedGame:
    ...
    var f := FileAccess.open_compressed(_get_path(_read_latest_name(id)), FileAccess.READ, _COMPRESSION)
    if FileAccess.get_open_error():
        return null
...
# Restores a valid file structure in the save directory. For example, after the game was closed or crashed while saving.
...
# This is the only method that should know about the specific time format.
# Other methods just assume this is a lexicographically ordered string.
...
func _new_file_name(id: UUID) -> String:
    return "%s%03d_%s" % [
        ...
        # Include milliseconds to avoid collisions of the name generated within the same second.
