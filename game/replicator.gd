...
# It handles model getting and setting, and replicating it to all clients. It DOES NOT handle save game specifics.
# To implement saving, get the model using Replicator and save it using desired method.
#
...
# It is a bit complicated because we need to make sure all parents are stored first and there are no duplicates.
class NodeAppender:
    var container: Node
