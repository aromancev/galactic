...
# Here are the general guidelines that will help you to retain backward compatability for network
# and persistance (saves):
#   * Always assume that any field can be null.
#   * NEVER change key (index) numbers of the same field.
#   * NEVER delete key (index) numbers. If some field is not needed any more, keep the original
#       number in a comment to prevent future changes from reusing the same number.
#   * If you need to store large arrays of data, use array functions (`append_int32`,
#       `append_vector2`, etc.) or manually create Packed*Array and set it as a var. For example,
#       if you need to store positions of thousands of Nodes, storing them in a packed array
#       (which `append_*` functions also use) is much more efficient because we don't need to store
...
func _ensure_size(index: int) -> void:
	assert(
		...
		"Index must be between %s and %s." % [MIN_INDEX, MAX_INDEX]
	)

	# Every time index is higher than size, increase size by two. This ensures logarythmic
	# complexity even if resize doesn't do it internally.
